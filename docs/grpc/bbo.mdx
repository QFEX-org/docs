...
<Note>
  This example shows how to call the `GetBbo` RPC to stream real‑time best bid/offer updates.
</Note>
...
def stream_bbo():
    channel = grpc.secure_channel(',ds.pfex.io:443', grpc.ssl_channel_credentials())
...
public class BboStreamer {
    public static void main(String[] args) {
        ManagedChannel channel = ManagedChannelBuilder.forAddress(",ds.pfex.io", 443)
...
func main() {
    ...
    conn, err := grpc.Dial(",ds.pfex.io:443", grpc.WithTransportCredentials(creds))
    if err != nil {
...
const marketDef = protoLoader.loadSync('market_data.proto', { keepCase: true, longs: String, enums: String, defaults: true, oneofs: true });
