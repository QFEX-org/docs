...
ws.on("open", () => {
  ...
  ws.send(JSON.stringify({ type: "get_user_trades", params: { limit: 10, offset: 0 } }));
});
...
<Note>
  Only **executed trades (fills)** are returned.  
  Open orders that have not filled are excluded.
</Note>
